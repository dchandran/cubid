...
/***/ function(module, exports, __webpack_require__) {
	...
	var SplitPane = __webpack_require__(163);
	var PartsView = __webpack_require__(168);
	var NetworkView = __webpack_require__(169);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SplitPane = __webpack_require__(164);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var _Pane = __webpack_require__(165);
	...
	var _reactVendorPrefix = __webpack_require__(166);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var PartsView = function (_React$Component) {
	  _inherits(PartsView, _React$Component);

	  function PartsView() {
	    _classCallCheck(this, PartsView);

	    return _possibleConstructorReturn(this, Object.getPrototypeOf(PartsView).apply(this, arguments));
	  }

	  _createClass(PartsView, [{
	    key: 'render',
	    value: function render() {
	      ...
	      return _react2.default.createElement(
	        'div',
	        { style: divStyle, 'class': 'resizable' },
	        _react2.default.createElement(
	          'svg',
	          { width: '100%', height: '100%', viewBox: '0 0 20 10' },
	          _react2.default.createElement('polygon', { points: '0,10 20,10 10,0', style: svgStyle })
	        )
	      );
	    }
	  }], [{
	    ...
	    value: function propTypes() {
	      return {
	        polygon: _react2.default.PropTypes.string.isRequired,
	        fillColor: _react2.default.PropTypes.string.isRequired,
...
/***/ function(module, exports, __webpack_require__) {
	...
	var d3Network = {
	    ...
	    create: function create(el, state) {
	        var width = el.clientWidth;
	        var height = el.clientHeight;
	        var self = this;

	        self.svg = d3.select(el).append('svg').attr('class', 'd3').attr('width', width).attr('height', height);

	        // Lasso functions to execute while lassoing
	        var lasso_start = function lasso_start() {
	            self.lasso.items().attr("r", 3.5) // reset size
	        ...
	        var lasso_draw = function lasso_draw() {
	            ...
	            self.lasso.items().filter(function (d) {
	        ...
	        var lasso_end = function lasso_end() {
	            ...
	            self.lasso.items().style("fill", function (d) {
	            ...
	            }).classed({ "not_possible": false, "possible": false }).attr("r", 7);

	            // Reset the style of the not selected dots
	            self.lasso.items().filter(function (d) {
	                return d.selected === false;
	            }).classed({ "not_possible": false, "possible": false }).attr("r", 3.5);
	        };

	        // Create the area where the lasso event can be triggered
	        var lasso_area = self.svg.append("rect").attr("width", width).attr("height", height).style("opacity", 0);

	        // Define the lasso
	        self.lasso = d3.lasso().closePathDistance(75) // max distance for the lasso loop to be closed
	    ...
	    getAlignmentBounds: function getAlignmentBounds(vs, c) {
	        var os = c.offsets;
	        if (c.axis === 'x') {
	            var x = vs[os[0].node].x;
	            c.bounds = new cola.vpsc.Rectangle(x, x, Math.min.apply(Math, os.map(function (o) {
	                return vs[o.node].bounds.y - 20;
	            })), Math.max.apply(Math, os.map(function (o) {
	                return vs[o.node].bounds.Y + 20;
	            })));
	        } else {
	            var y = vs[os[0].node].y;
	            c.bounds = new cola.vpsc.Rectangle(Math.min.apply(Math, os.map(function (o) {
	                return vs[o.node].bounds.x - 20;
	            })), Math.max.apply(Math, os.map(function (o) {
	                return vs[o.node].bounds.X + 20;
	            })), y, y);
	        }
	    ...
	    update: function update(el, state) {
	        ...
	        d3.json("alignmentconstraints.json", function (error, graph) {
	            ...
	            d3cola.nodes(graph.nodes).links(graph.links).constraints(graph.constraints).start(10, 10, 10);

	            var link = svg.selectAll(".link").data(graph.links).enter().append("line").attr("class", "link");

	            var guideline = svg.selectAll(".guideline").data(graph.constraints.filter(function (c) {
	                return c.type === 'alignment';
	            })).enter().append("line").attr("class", "guideline").attr("stroke-dasharray", "5,5");
	            ...
	            d3cola.on("tick", function () {
	                ...
	                guideline.attr("x1", function (d) {
	                    return self.getAlignmentBounds(graph.nodes, d).x;
	                }).attr("y1", function (d) {
	                    return d.bounds.y;
	                }).attr("x2", function (d) {
	                    return d.bounds.X;
	...
	var NetworkView = function (_React$Component) {
	    _inherits(NetworkView, _React$Component);

	    function NetworkView() {
	        _classCallCheck(this, NetworkView);

	        return _possibleConstructorReturn(this, Object.getPrototypeOf(NetworkView).apply(this, arguments));
	    }
	    ...
	    _createClass(NetworkView, [{
	        ...
	        value: function render() {
	            ...
	            return _react2.default.createElement('div', { className: 'NetworkView', style: divStyle });
	        }
	    }], [{
	        key: 'propTypes',
	        value: function propTypes() {}
